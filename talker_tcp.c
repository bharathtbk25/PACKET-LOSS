...
#define VERSION "$Id: talker_tcp.c 6 2007-05-31 16:28:28Z  $"

void usage()
{
  fprintf(stderr, "usage: talker_tcp [-v] [-h] [-s source_port] [-d destination_port] [-c count] [-t interval] hostname\n");
  fprintf(stderr, "       version: %s\n", VERSION);
  fprintf(stderr, "     -v  verbose (up to 2 times)\n");
  fprintf(stderr, "     -h  this page\n");
  fprintf(stderr, "     -s  source TCP port (default any)\n");
  fprintf(stderr, "     -d  destination TCP port (default %d)\n", SERVERPORT);
...
int main(int argc, char *argv[])
{
  ...
  unsigned short sport=0; //the source port used to send the packet
  ...
  while ((opt = getopt(argc, argv, "vhs:d:i:c:t:")) != EOF) {
    switch (opt) {
    case 'h':
      usage();;
      exit(-1);
  ...
  //only need to bind if we want to specify source port
  if(sport){
